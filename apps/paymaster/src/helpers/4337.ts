/**
 * File from v7 of @account-abstraction/utils which is not on npm yet...
 * :sadge:
 */
...
const { defaultAbiCoder, hexConcat, hexDataLength, hexDataSlice, hexlify, hexZeroPad, keccak256 } = utils;
...
export function mergeValidationData(
  ...
  return {
    aggregator:
      paymasterValidationData.aggregator !== AddressZero ? SIG_VALIDATION_FAILED : accountValidationData.aggregator,
...
export function packUserOp(op: UserOperation): PackedUserOperation {
  ...
  if (!op.paymaster || op.paymaster == null) {
    paymasterAndData = '0x';
  } else {
